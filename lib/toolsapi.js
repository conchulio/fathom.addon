...
var handleremoteapi = exports.handleremoteapi = function(callback, method, params, manifest) {
    console.info("In remoteapi",params)
    switch (method) {
    case "start":
        ...
        if (!discoveryserver) {
            ...
            console.log("There's already a server, I just do nothing :)")
            callback({}, true);
        }
        break;
    ...
    case "makereq":
        ...
        var timeout = (params.length === 4 ? params[3] : undefined);
        console.log('makereq params', JSON.stringify(params))
        console.log('makereq timeout',timeout)

        protoapi.exec(function(id) {
            if (id.error) {
                console.log('ahh, an error occured')
                return callback(id);
            }
 ...
 */ 
 var exec = exports.exec = function(callback, req, manifest) {
    ...
    if (req.submodule === 'iperf' || req.submodule === 'ping') {
        ...
        // remote API stuff
        console.info("exec calls handleremoteapi")
