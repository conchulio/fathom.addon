...
rpc.prototype.connect = function(callback) {
  ...
  if (this.isserver) {
    ...
    that.makesocketreq(function(sid) {
      console.log('jsonrpc '+this.debug_id+'  sid in connect', sid)
      if (sid.error) {
        console.log('jsonrpc '+this.debug_id+' ', 'error makesocketreq')
        return callback(sid, true);
      }

      console.log('jsonrpc '+this.debug_id+'  setting socketid to something, previous one', that.socketid)
      that.socketid = sid;
      console.log('jsonrpc '+this.debug_id+'  setting socketid to something, new one', that.socketid)
      var ready = function(res) {
        if (res.error) {
            console.log("jsonrpc "+this.debug_id+"","udp close method error beginning")
            that.makesocketreq(function() {},
                   'close',[that.socketid]);
            console.log('jsonrpc '+this.debug_id+'  setting socketid to -1, ready part, previous one', this.socketid)
            that.socketid = -1;
            that.reqid = 0;
        }   
        callback(res, true);
...
rpc.prototype.makereq = function(callback, method, params, module, urlparams, timeout) {
  ...
  // open
  that.makesocketreq(function(sid) {
    console.log('jsonrpc '+this.debug_id+'  socketid sid',sid)
    if (sid.error)
      return callback(sid,true);
    ...
    that.makesocketreq(function(res) {
      if (res.error) {
          console.log("jsonrpc "+this.debug_id+"","udp close method error")
          that.makesocketreq(function() {},'close',[sid]);    
    ...
    that.makesocketreq(function(res, done) {
      ...
      try {
        ...
        if (done) {
      // cleanup
      console.log("jsonrpc "+this.debug_id+"","udp close method cleanup")
