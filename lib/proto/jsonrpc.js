...
rpc.prototype.makeUdpReqAndRecycle = function(callback, msgobj, timeout) {
  var that = this;
  ...
  this.getSendingSocket(function(sid) {
    that.makesocketreq(function(res) {
      if (res.error) {
        console.error("jsonrpc "+that.debug_id,"udpSendTo error",res.error)
        that.makesocketreq(function() {},'close',[sid]);    
        return callback(res,true);
      }
      sendingSocket.callbacks[msgobj.id] = sendingSocket.callbacks;
      console.log('All callbacks in JSONRPC now', callbacks)
      // callback(sendingSocket);
      // recvfromstart
      that.makesocketreq(function(res, done) {
        if (res.error) {
          console.error("jsonrpc "+that.debug_id, 'error in recvfromstart: '+res.error);
          // callback(res,done);
        } else if (res.data) {
          try {
            ...
            if (resobj.error) {
              ...
              if (callback = sendingSocket.callbacks[id]) {
                console.log("jsonrpc "+that.debug_id, 'done is', done)
                callback({result : resobj.result, address : res.address},done);
                if (done) {
                  console.log("jsonrpc "+that.debug_id, 'done is true, removing callback');
                  // if done then delete request id from callbacks
                  // delete sendingSocket.callbacks[id];
                }
              } else {
                console.error("jsonrpc "+that.debug_id, "Got response but there was no request, response "+resobj);
              }
            }
          } catch (e) {
      ...
      console.log("jsonrpc "+this.debug_id+" setting timeout to",timeout)
      timers.setTimeout(function() {
          console.log("jsonrpc "+this.debug_id+" timeout",sid)
          // that.makesocketreq(function() {},'udpRecvStop',[sid]);
          // that.makesocketreq(function() {},'close',[sid]);
...
rpc.prototype.makereq = function(callback, method, params, module, urlparams, timeout) {
  ...
  switch (this.proto) {
    ...
    case "udp":
    case "multicast":
      this.makeUdpReqAndRecycle(callback, msgobj, timeout);
      // open
      // that.makesocketreq(function(sid) {
