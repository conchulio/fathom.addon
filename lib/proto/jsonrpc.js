...
rpc.prototype.getSendingSocket = function(callback, proto, args) {
  ...
  if (sendingSocket === null) {
    ...
    that.makesocketreq(function(sid) {
      ...
      if (sid.error) {
        console.error('jsonrpc', 'getSocket create new socket')
...
rpc.prototype.makeReqAndRecycle = function(callback, msgobj, timeout) {
  ...
  that.getSendingSocket(function(sid) {
    ...
    console.log(msgobj.id,'getSendingSocket callback, callbacks in JSONRPC now', sendingSocket.callbacks.length);

    that.makesocketreq(function(res, done) {
      // console.log("jsonrpc Yeah, callback called!!!");
      // console.log(msgobj.id,'udpSendTo callback');
      ...
      timers.setTimeout(function() {
        ...
        // console.log("jsonrpc", "Removing callback for ",msgobj.id);
...
rpc.prototype.makereq = function(callback, method, params, module, urlparams, timeout) {
  ...
  this.rpcreqid += 1;
  var msgobj = new JsonReq(this.objectId+"-"+this.rpcreqid, method, params);
  var msg = JSON.stringify(msgobj);
...
rpc.prototype.close = function(callback) {
  if (this.socketid && this.socketid >= 0) {
    // Don't close socket anymore because it's recycled
    // if (sendingSocket && typeof(sendingSocket.sid) !== 'undefined' && sendingSocket.sid !== this.socketid) {
    console.log('this.socketid', this.socketid, 'sendingSocket.sid', sendingSocket.sid)
    if (this.isserver) {
      this.makesocketreq(function() {},'udpRecvStop',[this.socketid]);

      // The following line is new in this implementation
      this.makesocketreq(function() {},'close',[this.socketid]);
    } 
    // else {
    //   sendingSocket = null;
    // }
      // console.log("It's a different socketid so we can close it, sendingSocket.sid", sid, "this.socketid", this.socketid);
      // this.makesocketreq(function() {},'close',[this.socketid]);
  }
  this.socketid = -1;
  this.reqid = 0;
